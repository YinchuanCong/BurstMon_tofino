...
control SwitchIngress(
    ...
    action set_equal_sign_1(bit<1> sign) {
        ig_md.equal_sign_0 = sign;
        ig_md.equal_sign_1 = sign;
    }
    table time_equal_table_1 {
        key = {
            ig_md.A_W_T_1 : exact;  
    ...
    action set_equal_sign_2(bit<1> sign) {
        ig_md.equal_sign_2 = sign;
        ig_md.equal_sign_3 = sign;
    }
    table time_equal_table_2 {
        key = {
            ig_md.A_W_T_2 : exact;  
            ig_md.time[7:0] : exact;
        }
    ...
    action set_equal_sign_3(bit<1> sign) {
        ig_md.equal_sign_4 = sign;
        ig_md.equal_sign_5 = sign;
    }
    table time_equal_table_3 {
        key = {
            ig_md.A_W_T_3 : exact;  
            ig_md.time[7:0] : exact;
        }
    ...
    action set_equal_action_0(bit<1> sign){
        ig_md.equal_sign = sign;
    }
    action set_equal_action_2(bit<1> sign){
        ig_md.equal_sign = sign;
    }    
    action set_equal_action_4(bit<1> sign){
        ig_md.equal_sign = sign;
    }
    table set_equal_table {
        key = {
            ig_md.time_conv : exact;
            ig_md.equal_sign_0 : exact;
            ig_md.equal_sign_2 : exact;
            ig_md.equal_sign_4 : exact;
        }
        actions = {
            set_equal_action_0;
            set_equal_action_2;
    ...
    action set_abs_delta(int<16> abs_delta){
    ...
    action set_A_W_T_action(){
        ig_md.A_W_T_1 = min(ig_md.t0, ig_md.t1)[15:8];
        ig_md.A_W_T_2 = min(ig_md.t2, ig_md.t3)[15:8];
        ig_md.A_W_T_3 = min(ig_md.t4, ig_md.t5)[15:8];
    }
    ...
    apply{
    ig_md.time = ig_prsr_md.global_tstamp[31:16];// 2^16 ns=65.535 us
    // ig_md.windownum = (bit<4>)(ig_md.time >> 12);

    set_time_conv_table.apply();

    // ig_md.time_conv = (bit<2>)(ig_md.windownum & 0b11);
    // if(ig_md.time_conv == 3){
    //     ig_md.time_conv = 0;
    // }
    ...
    set_A_W_T_action();   //获取当前写操作时间窗口
    time_equal_table_1.apply();
    time_equal_table_2.apply();
    time_equal_table_3.apply();
    set_equal_table.apply();
    ...
    ig_md.delta = ig_md.bridge_hdr.len_i - ig_md.bridge_hdr.len_i_1;
    ...
    ig_md.bridge_hdr.S = (int<16>)min(ig_md.S1, ig_md.S2);
    // ig_md.S = min(ig_md.S1, ig_md.S2);
    
    // ig_md.bridge_hdr.delta = (int<16>)ig_md.delta;
    ig_md.bridge_hdr.time = (int<16>)ig_md.time;
    // ig_md.bridge_hdr.S = (int<16>)ig_md.S;
...
control SwitchEgress(
    ...
    eg_md.ac_md.int_m2 = eg_md.ac_md.t;
    eg_md.ac_md.int_m2 = eg_md.bridge_hdr.time;
    get_log_int_m2_table.apply();

    eg_md.ac_md.sign_z = 0x0000;
    eg_md.ac_md.n = eg_md.ac_md.log_int_m1 + eg_md.ac_md.log_int_m2;
    get_abs_z_table.apply();
    eg_md.ac_md.z = eg_md.ac_md.z | eg_md.ac_md.sign_z;
    eg_md.ac_md.at = eg_md.ac_md.z;  // a*t  is INT16
    // checked

    eg_md.ac_md.z = eg_md.ac_md.at - eg_md.bridge_hdr.S;
    if (eg_md.ac_md.z[15:15]==0) {
        eg_md.ac_md.at_s = eg_md.bridge_hdr.S - eg_md.ac_md.at;
    } else {
        eg_md.ac_md.at_s =  eg_md.ac_md.at - eg_md.bridge_hdr.S ;
    }
    // checked
    ...
    eg_md.ac_md.sign_z = 0x0000;
    eg_md.ac_md.n = eg_md.ac_md.log_int_m1 + eg_md.ac_md.log_int_m2; // checked
    // eg_md.ac_md.n = hdr.calc.a; // for debug 4449-> 56792?
    get_abs_z_table2.apply();
    eg_md.ac_md.f_sqrt_s_t_1 = eg_md.ac_md.z ; // 1/sqrt(s*(t-1))  float16
    //
    ...
    eg_md.ac_md.log_int_m2 = eg_md.ac_md.f_sqrt_s_t_1; // is a bit16 float >1
    get_log_int_m2_table3.apply();
    // eg_md.ac_md.sign_z = 0x0000;

    eg_md.ac_md.n = eg_md.ac_md.log_int_m1 + eg_md.ac_md.log_int_m2;
    get_abs_z_table3.apply(); // wanchen,to eg_md.ac_md.z
    
    // eg_md.ac_md.z is the final anwser

    // eg_md.ac_md.score = eg_md.ac_md.z;

    // // hdr.calc.a = eg_md.ac_md.log_int_m1;
    // // hdr.calc.s = eg_md.ac_md.log_int_m2;
    // // hdr.calc.t = eg_md.ac_md.n;
    // hdr.calc.score = eg_md.ac_md.score;
    }
