...
class DataPlane:
    def __init__(self,d,w,timestep):
        ...
        MemCost:
            ...
            s      : {(self.s.V.dtype.itemsize)*(self.s.d*self.s.w)} bits, {(self.s.V.dtype.itemsize)*(self.s.d*self.s.w)/8/1024} KB
    ...
    def Replay(self,path='./output/websearch25_10000_delta_score.csv'):
        ...
        flows = pd.read_csv("./data/websearch25.csv",header=None,names=['ID', 'len', 't', 'Flag'])# 流数据进行重放
...
class ControlPlane:
    ...
    def rebuild(self,filepath = 'output/webseach25_10000_delta_score.csv',res_file='websearch25_res.csv'):
        ...
        with open(res_file, "a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(['ID','packets','windowlen','cos',"ed",'energy'])
        ...
        for ID in list(set(flows['ID'].tolist())):
            ...
            # rebuild
            flow = flows[(flows['ID'] == ID) & (flows['delta'] > self.T )].copy()
            ...
            align_raw_data = raw_data.reindex(full_index,fill_value=0).fillna(0)
            align_recovered = recovered.reindex(full_index,fill_value=0).fillna(0)
            ...
            print(ID,packets, windowlen,cos,ed,energy)
...
if __name__ == '__main__':
    ...
    w = 1024*4
    timestep = 1e4 
    outpacket_file = f'output/delta_websearch25_{int(timestep)}_{d}_{w}_outpacket.csv'
    ...
    res_file = f'output/delta_websearch25_{T}_{d}_{w}_{timestep}_res.csv'
