...
header calc_h {
    int<16> a; // Flow ID (represents the original five-tuple during replay)
    int<16> s; // Packet length
#ifdef RAW_TIMESTAMP_REPLAY
    bit<48> timestamp; // Original CSV timestamp in nanoseconds
#else
    int<16> t; // Precomputed logical time window
    int<16> score; // Burst score
#endif
}
